 ...
 */
public class HomeDropStrategy extends TransactionalDropStrategy {

	/**
	 * Constructor.
	 */
	public HomeDropStrategy() {
	}

	@Override
	public String getLabel() {
		return "Home drop to create HomeInstance";
	}

	@Override
	public String getDescription() {
		return "Home drop to create CCMPart";
	}
	...
	@Override
	public String getID() {
		return com.zeligsoft.domain.dds4ccm.ui.Activator.PLUGIN_ID + ".HomeToHomeInstanceDrop";
	}
	...
	protected Command doGetCommand(Request request, EditPart targetEditPart) {
		...
		List<org.eclipse.uml2.uml.Class> handledDroppedObjects = getDroppedHomes(dropReq);
	...
	protected Command getCreateAndDropObjectCommand(EObject droppedObject, Component targetComponent, Point location,
		...
		IElementType type = ZDLElementTypeUtil.getElementType(targetComponent, CCMNames.HOME_INSTANCE);
	...
	protected List<org.eclipse.uml2.uml.Class> getDroppedHomes(Request req) {
		List<EObject> droppedObjects = getSourceEObjects(req);
		List<Class> result = new ArrayList<Class>();
		if (droppedObjects != null) {
			for (EObject droppedObject : droppedObjects) {
				if (ZDLUtil.isZDLConcept(droppedObject, CCMNames.HOME)) {
					result.add((org.eclipse.uml2.uml.Class) droppedObject);
				}
			}
		}
		return result;
	...
	protected boolean canHandleRequest(List<org.eclipse.uml2.uml.Class> droppedObjects, EObject targetElement) {
