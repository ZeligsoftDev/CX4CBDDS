 ...
 *
 */
package com.zeligsoft.domain.dds4ccm.ui.properties;
...
import com.zeligsoft.cx.ui.properties.sections.AbstractCXPropertySection;
...
public class DomainPropertySection extends AbstractCXPropertySection {
	...
	private void createWidgetsForProperties(Composite composite) {
		...
		String modelType = UML2Util.EMPTY_STRING;
		if(ZDLUtil.isZDLConcept(root, DDS4CCMNames.DDS4_CCMMODEL)) {
			EnumerationLiteral mtype = (EnumerationLiteral) ZDLUtil.getValue(root, DDS4CCMNames.DDS4_CCMMODEL, DDS4CCMNames.DDS4_CCMMODEL__MODEL_TYPE);
			modelType = mtype.getName();
		}
		...
		for (int i = concepts.length - 1; i >= 0; i--) {
			...
			while (attrItor.hasNext()) {
				...
				CXPropertyDescriptor descriptor = CXPropertyDefinitionManager.INSTANCE.getPropertyDescriptor(context,
						concept, property);
				...
				if (descriptor.getPropertyDefinition() != null) {
					...
					String visibleModelType = descriptor.getPropertyDefinition().getVisibleModelType();
					if (visibleModelType != null) {
						if (!"ALL".equals(visibleModelType) && !visibleModelType.equals(modelType)) { //$NON-NLS-1$
							continue;
						}
					}
				}
			...
			for (CXPropertyDescriptor descriptor : propertyDescriptors) {
				...
				List<CXCustomPropertySectionRegistryReader.CustomPropertySection> customSections = CXCustomPropertySectionRegistryReader.INSTANCE
						.getCustomPropertySections(descriptor.getContext(), descriptor.getConcept().getQualifiedName(),
								property.getName());
				...
				if (overrideSection != null) {
					widgets = overrideSection.createSection(composite, descriptor, property);
				} else if (property.isComposite()) {

					widgets = CXPropertiesWidgetFactory.createSectionCompositeType(composite, descriptor);
				} else {
					if (property.getType() instanceof PrimitiveType) {
						...
						if (type.equals(String.class.getSimpleName()) || type.equals(Integer.class.getSimpleName())) {
							widgets = CXPropertiesWidgetFactory.createSectionForStringType(composite, descriptor);
						} else if (type.equals(Boolean.class.getSimpleName())) {
							CXPropertiesWidgetFactory.createSectionForBooleanType(composite, descriptor);
						}

					} else if (property.getType() instanceof Enumeration) {
						widgets = CXPropertiesWidgetFactory.createSectionForEnumerationType(composite, descriptor);
					} else {
						widgets = CXPropertiesWidgetFactory.createSectionForReferenceType(composite, descriptor);
					}
				}
				...
				for (CXCustomPropertySectionRegistryReader.CustomPropertySection section : customSections) {
					if (section.shouldAppend()) {
						widgets = section.getSection().createSection(composite, descriptor, property);
						addWidgetListener(widgets);
					}
				}
			}
	...
	private void addWidgetListener(Map<String, Control> widgets) {
		if (widgets != null) {
			if (widgets.containsKey(CXPropertiesWidgetFactory.PROPERTY_EDIT_BUTTON)) {
				Control control = widgets.get(CXPropertiesWidgetFactory.PROPERTY_EDIT_BUTTON);
			...
			if (widgets.containsKey(CXPropertiesWidgetFactory.PROPERTY_DELETE_BUTTON)) {
				((ToolBar) widgets.get(CXPropertiesWidgetFactory.PROPERTY_DELETE_BUTTON)).getItem(0)
	...
	private Set<Property> getCommonPropertySet() {
		...
		Iterator<Class> iter = ZDLUtil.getZDLConcepts(getSelectedEObjects().get(0)).iterator();
		while (iter.hasNext()) {
