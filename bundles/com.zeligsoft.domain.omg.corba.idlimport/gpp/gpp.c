...
#ifdef WIN_NT              /* WIN NT settings -- IN PRISMTECH, WE NO LONGER USE THESE SETTINGS */
#define popen   _popen
...
/* controls whether discovered macros will be cleared when returning to the
   top-level file */
struct MACRO * user_macros;
int nuser_macros = 0;
int nuser_alloced = 0;
int ClearDiscoveredMacros = 0;
int IncludeLevel = 0;
typedef struct processed_files_s
{
	char * filename;
	struct processed_files_s * next;
} processed_files_t;
static processed_files_t * processed_files = NULL;
...
void PopSpecs(void)
{
  ...
  if( --IncludeLevel <= 0
   && ClearDiscoveredMacros )
  {
	  free( macros );
	  macros = malloc( nuser_alloced * sizeof( *macros ) );
	  if( macros )
	  {
		  nalloced = nuser_alloced;
		  nmacros = nuser_macros;
		  memcpy( macros, user_macros, nuser_alloced * sizeof( *macros ) );
	  }
  }
...
void usage(void) {
  ...
  fprintf(stderr," -Z : Zeligsoft comment mode, see Bug 15592\n");
  ...
  fprintf(stderr," -e : enter the name of include file to skip\n");
  ...
  fprintf(stderr," --multi-file-import : process the temp file used to import multiple IDL files\n");
...
#define DEPRECATED_WARNING fprintf(stderr, "gpp: warning: deprecated option `%s'; use `-%s' instead\n", *arg, *arg)
    ...
    if( ! strcmp(*arg, "--multi-file-import") )
    {
    	ClearDiscoveredMacros = 1;
    	continue;
    }
    if (strcmp(*arg, "--includemarker") == 0) {
      if (!(*(++arg))) {
    ...
    case 'Z':
      ...
      S->User=KUser; S->Meta=KMeta;
      S->preservelf=1;
      add_comment(S,"SSS",my_strdup("/*"),my_strdup("*/"),0,0);
      add_comment(S,"SSS",my_strdup("//"),my_strdup("\n"),0,0);
    ...
    case 'm':
      autoswitch=1; break;
    case 'e':
    	/* Read include files names to not process */
        if (nexcludedfiles==MAXINCL)
    	bug("too many excluded files");
        if ((*arg)[2]==0) {
        	if (!(*(++arg)))
				{usage(); exit(EXIT_FAILURE);}
        	excludedfiles[nexcludedfiles++]=my_strdup(*arg);
        }
        else
        {
        	excludedfiles[nexcludedfiles++]=my_strdup((*arg)+2);
        }
    	break;
    default:
      ishelp=1;
...
int DoArithmEval(char *buf,int pos1,int pos2,int *result)
{
  ...
  if (SpliceInfix(buf,pos1,pos2,"=~",&spl1,&spl2)) {
    ...
    if (!DoArithmEval(buf,pos1,spl1,&result1)||
      ...
      str2[pos2-spl2]='\0';
#if HAVE_FNMATCH_H // MCFEE - hopefully they put globbing suport in sometime soon :)
...
/**
 * \param expparams If no parameters are expected, then it is ok to skip comments, otherwise
 *                  they should be processed.  This is because the argument to #include (as
 *                  well as other directives) could be a quoted (#include "file.h") and ignoring
 *                  comments causes strings to be ignored as well.
 */
int comment_or_white(int start,int end,int cmtmode, int expparams)
{
  char c;

  while (start<end) {
    SkipPossibleComments(&start,cmtmode, expparams ? 1 : 0);
    if (start<end) {
      c=getChar(start++);
      if ((c!=' ')&&(c!='\n')&&(c!='\t')) return 0;
    }
...
int exclude_IncludeFile(char * file_name)
{
	int i;
	for (i=0; (i<nexcludedfiles);i++) {
		//printf("looking for exclude file %s compare to %s \n", file_name, excludedfiles[i]);
		if (strcmp(file_name, excludedfiles[i]) == 0 || wildcmp(file_name, excludedfiles[i]) == 1)
		{
			 //printf("found exclude file %s \n", file_name);
			 return 1;
	    }
	}
	return 0;
...
static void DoInclude(char *file_name)
{
  ...
  processed_files_t * this_node = ClearDiscoveredMacros ? malloc( sizeof( processed_files_t ) ) : NULL;
  if( this_node )
	  memset( this_node, 0, sizeof( *this_node ) );

  /* See if this include file is to be excluded */
  if (exclude_IncludeFile(file_name))
  {
      char fileMarker[256];
      strcpy(fileMarker, "#excluded_file \"");
      strcat(fileMarker, file_name);
	  strcat(fileMarker, "\"\n");
	  sendout(fileMarker, strlen(fileMarker), 0);
	  return;
  }
      ...
      )
  {
    ...
    if( this_node
     && f )
    	this_node->filename = strdup( file_name );
  }
  ...
  for (j=0;(f==NULL)&&(j<nincludedirs);j++) {
    ...
    if (exclude_IncludeFile(incfile_name))
    {
      char fileMarker[256];
    ...
    if( this_node
     && f )
    	this_node->filename = strdup( incfile_name );
  }
  ...
  if( this_node
   && this_node->filename )
  {
	  this_node->next = processed_files;
	  processed_files = this_node;
  }
  ...
  /* Only process the rest of the file if it has not already been processed */
  int should_process = 1;
  if( this_node )
  {
	  processed_files_t * node;
	  for( node = processed_files; should_process && node; node = node->next )
		  if( node != this_node
		   && ! strcmp( node->filename, this_node->filename ) )
			  should_process = 0;
  }
...
int ParsePossibleMeta(void)
{
  ...
  /* Bug 15592: Don't process pre-processor directives when parsing a comment */
  if( C->in_comment)
	return -1;
  ...
  if (idequal(C->buf+cklen,nameend-cklen,"define"))      /* check identifier */
    ...
    { id=20; expparams=1; }
  else if (idequal(C->buf+cklen,nameend-cklen,"pragma"))
    { id=21; expparams=2; }
  else return -1;
  ...
  if ((nparam==2)&&iswhitesep(S->Meta.mArgSep))
    if (comment_or_white(p2start,p2end,FLAG_META,expparams)) nparam=1;
  if ((nparam==1)&&iswhitesep(S->Meta.mArgS))
    if (comment_or_white(p1start,p1end,FLAG_META,expparams)) nparam=0;
 ...
 case 21: { /* PRAGMA */
    ...
    fmt=ProcessText(C->buf + p1start,
		    (nparam == 2 ? p2end : p1end) - p1start,
		    FLAG_META);
    sendout("#pragma ", strlen("#pragma "), 0);
    sendout(fmt, strlen(fmt), 0);
    sendout("\n", strlen("\n"), 0);
    free(fmt);
  }
    break;
...
#else
    escapedfilename = my_strdup(filename);
#endif

    /* bug#16375 + 16375 -
     * We want to strip out path info leading up
     * to the actual filename, otherwise gpp treats
     * filenames with and without the path as separate
     * files, creating separate entries for them
       */
    char * slashInstance = NULL;
    slashInstance = strrchr(escapedfilename, SLASH);
    if (slashInstance != NULL){
      strcpy(escapedfilename, slashInstance+1);
    }
...
int main(int argc,char **argv)
{
  ...
  /* Copy the user's macros so they can be reset between top-level files */
  /* The include marker at the top of the file */
  if( ClearDiscoveredMacros )
  {
	  nuser_macros = nmacros;
	  nuser_alloced = nalloced;
	  user_macros = malloc( nuser_alloced );
	  if( user_macros )
		  memcpy( user_macros, macros, nuser_alloced );
  }
