...
public class TraceKey extends AbstractHierarchicalKey<TraceKey> {
	...
	public TraceKey(org.eclipse.uml2.uml.NamedElement element) {
		...
		EAnnotation annotation = element.getEAnnotation("@tao_idl_generated"); //$NON-NLS-1$
		elementName = element.getQualifiedName();
		if(annotation != null) {
			sourceElement = annotation.getDetails().get("source"); //$NON-NLS-1$
			ruleName = annotation.getDetails().get("transform"); //$NON-NLS-1$
		}
	}
