...
bool KillProcess(CF::ExecutableDevice::ProcessID_Type id, ExecInfoList& execInfoList, std::string &strErr)
{
	...
	do
	{
		nRet = waitpid ((pid_type) id, &status, WNOHANG);
		if (nRet == (pid_type) id)
		{
			// Child has gone
			return true;
	...
	status = kill((pid_type) id, SIGTERM); // 0 is returned on success
	...
	{
	   nRet = waitpid((pid_type) id, &status, WNOHANG); // Check if process is terminated
	   ...
	   {
		   // No status available yet, or call was interrupted.
