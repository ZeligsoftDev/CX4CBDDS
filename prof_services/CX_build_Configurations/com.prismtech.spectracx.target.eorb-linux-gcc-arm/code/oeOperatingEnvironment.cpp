...
void* Shutdown_Thread(void* data)
{
	...
	if (OperatingEnvironment::status == OE_STATUS_RUNNING)
	{
		...
		OE_CATCH_ANY
		{
			std::string errorMsg = "oeOperatingEnvironment : Exception caught during orb->shutdown().";
			printErrorMsg(errorMsg.c_str());
			return NULL;
		}
		OE_END_TRY
	}
...
bool activate_component(CORBA::ORB_var orb, PortableServer::Servant servant OE_ENV_ARGN)
{
	...
	{
		std::string errorMsg = "Failed to narrow the POA.";
		printErrorMsg(errorMsg.c_str());
		return false;
	}
		...
		CORBA::Object_var component OE_ENV_ARGN)
{
	...
	if(CORBA::is_nil(obj))
	{
		std::string errorMsg = "Failed to resolve the naming context.";
		printErrorMsg(errorMsg.c_str());
		return false;
	}
	...
	if(CORBA::is_nil(nc))
	{
		std::string errorMsg = "Failed to narrow the naming context.";
		printErrorMsg(errorMsg.c_str());
		return false;
	}
	CosNaming::Name name;
