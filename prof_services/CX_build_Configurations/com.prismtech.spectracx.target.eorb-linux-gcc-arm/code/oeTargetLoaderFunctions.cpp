...
void* shutdown_thread(void* targetLoader)
{
	...
	OE_CATCH_ANY
	{
		std::string errorMsg = "oeTargetLoaderFunctions : Caught exception during targetLoader->shutdown().";
...
void register_process_termination_callback (const Target::TargetLoader_ptr targetLoader)
{
	...
	{
		std::string errorMsg = "oeTargetLoaderFunctions : Failed to create shutdown thread.";
		printErrorMsg(errorMsg.c_str());
	}
}
