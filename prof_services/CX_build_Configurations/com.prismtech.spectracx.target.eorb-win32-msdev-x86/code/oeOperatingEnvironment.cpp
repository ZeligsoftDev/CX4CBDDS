...
// allow the shutdown_thread to complete
...
void register_process_termination_callback()
{
	...
	pthread_t thread;
	int rc = pthread_create (&thread, NULL, shutdown_thread, (void *) 0);

	if( rc == 0) {
		if( SetConsoleCtrlHandler( (PHANDLER_ROUTINE) CtrlHandler, true ) == false) {
			std::cerr << "oeTargetLoaderFunctions : Failed to create control handler." << std::endl;
		}
	} else {
