...
int runInitOrFini (const char *file, void *libHandle, char *extName, void **sym_ptr)
{
   ...
   if ( OK == symFindByName (sysSymTbl, functionName, &address, &type))
   {
      *sym_ptr = address;
      result = 0;
   }
...
bool LoadFile(CF::FileSystem_ptr fs, const char *fileName, std::string deviceId, CF::LoadableDevice::LoadType loadKind, FileMap& fileMap, std::string& strErr OE_ENV_ARGN)
{
	...
	do
	{
		...
		{
			strErr = std::string("Error: fwrite failed to write to file: \"") + strTmpPath + std::string("\"");
	...
	if (fdX == 0)
	{
		strErr = std::string("Failed to open file: \"") + file + std::string("\"");
		std::cerr << strErr << std::endl;
		return false;
	}
	...
	if (mid == 0)
	{
		strErr = std::string("Failed to load file: \"") + std::string(fileName) + std::string("\"");
		std::cerr << strErr << std::endl;
		return false;
	}
...
bool UnloadFile(const char *fileName, FileMap& fileMap, std::string& strErr)
{
	...
	{
		strErr = std::string("The file name you are unloading was not loaded: \"") + std::string(fileName) + std::string("\"");
		std::cerr << strErr << std::endl;
		return false;
	}
	...
	{
		strErr = std::string("The file can not be unloaded: \"") + std::string(fileName) + std::string("\"");
		std::cerr << strErr << std::endl;
		return false;
	}
	else
	{
		if( OK != remove( fileMap[fileName].strLocalFilename.c_str() ) )
		{
			strErr = std::string("The file can not be deleted: \"") + fileMap[fileName].strLocalFilename + std::string("\"");
			std::cerr << strErr << std::endl;
			return false;
		}
	}
