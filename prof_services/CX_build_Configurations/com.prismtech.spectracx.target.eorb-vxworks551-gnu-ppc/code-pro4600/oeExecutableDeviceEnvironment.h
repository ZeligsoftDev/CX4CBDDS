...
public:
	...
	OEFileInfo (std::string filename, std::string localFilename, CF::LoadableDevice::LoadType loadType, MODULE_ID id) :
				strFilename (filename), strLocalFilename (localFilename), type (loadType), m_id (id), refCount (1), next (NULL) {};
};
...
public:
	...
	void add (std::string filename, std::string localFilename, CF::LoadableDevice::LoadType type, MODULE_ID id);
