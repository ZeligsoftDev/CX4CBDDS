...
CORBA::ORB_ptr oeOrbFunctions::init_orb(int argc, const char *argv[] OE_ENV_ARGN)
{
	...
	bool threadStackSpecified = false;
	...
	{
		if (strcmp (argv[i], "-ORBThreadStack") == 0 || strcmp (argv[i], "ORBThreadStack") == 0)
		{
			threadStackSpecified = true;
		}
	}

	int newArgc = threadStackSpecified ? argc : argc + 2;
	...
	if (!threadStackSpecified)
	{
		newArgv[argc] = new char[16];
		strcpy(newArgv[argc], "-ORBThreadStack");
		newArgv[argc + 1] = new char[6];
		strcpy(newArgv[argc + 1], "16384");
	}
	...
	OE_CATCH_ANY
	{
		...
		delete [] newArgv;
		OE_RETHROW_RETURN(CORBA::ORB::_nil());
	}
	OE_END_TRY
...
PortableServer::POA_ptr oeOrbFunctions::get_rootpoa (CORBA::ORB_ptr orb OE_ENV_ARGN)
{
	...
	OE_CHECK_ENV_RETURN(PortableServer::POA::_nil());
...
CosNaming::NamingContext_ptr oeOrbFunctions::get_naming_service (CORBA::ORB_ptr orb OE_ENV_ARGN)
{
	...
	OE_CHECK_ENV_RETURN(CosNaming::NamingContext::_nil());
