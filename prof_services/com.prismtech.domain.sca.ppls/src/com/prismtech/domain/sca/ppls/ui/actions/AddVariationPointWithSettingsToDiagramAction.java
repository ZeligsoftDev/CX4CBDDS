...
public class AddVariationPointWithSettingsToDiagramAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {	
		...
		ICommand command = new AbstractTransactionalCommand(TransactionUtil.getEditingDomain(eo),
				Messages.CreateVariationPointWithSettingsLabel,
			...
			protected CommandResult doExecuteWithResult(
				...
				if( ZDLUtil.isZDLConcept(eo, SCANames.SCAPLATFORM_ELEMENT_PART)
						|| ZDLUtil.isZDLConcept(eo, SCANames.PROPERTY_PROVIDER)
						|| ZDLUtil.isZDLConcept(eo, SCANames.SCAASSEMBLY)
						|| ZDLUtil.isZDLConcept(eo, SCANames.SCAFREE_STANDING_PORT)) {
		...
		try {
			...
			Activator.getDefault().error(Messages.CreateVariationPointWithSettingsError, e);
		}
	}
