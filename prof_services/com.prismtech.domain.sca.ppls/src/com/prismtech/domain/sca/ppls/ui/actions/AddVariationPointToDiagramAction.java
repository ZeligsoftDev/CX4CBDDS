...
public class AddVariationPointToDiagramAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {		
		...
		ICommand command = new AbstractTransactionalCommand(TransactionUtil.getEditingDomain(eo),
			...
			protected CommandResult doExecuteWithResult(
				...
				if( ZDLUtil.isZDLConcept(eo, SCANames.SCAPORT) 
						|| ZDLUtil.isZDLConcept(eo, SCANames.SCACOMPONENT_PART)
						|| ZDLUtil.isZDLConcept(eo, SCANames.SCANODE_PART)
						|| ZDLUtil.isZDLConcept(eo, SCANames.SCAOPERATING_SYSTEM_DEPENDENCY)
			            || ZDLUtil.isZDLConcept(eo, SCANames.SCAPROCESSOR_DEPENDENCY)
			            || ZDLUtil.isZDLConcept(eo, SCANames.SCACONNECTOR)) {
							...
							if( p.getAppliedProfile().getName().equals(PLMNames.VARIATION_POINT_PROFILE)) {
								for( Stereotype s : p.getAppliedProfile().getOwnedStereotypes()) {
									if( s.getQualifiedName().equals(PLMNames.VARIATION_POINT)) {
										...
										String elementName = (String) ZDLUtil.getValue(eo, ZMLMMNames.NAMED_ELEMENT, ZMLMMNames.NAMED_ELEMENT__NAME);
