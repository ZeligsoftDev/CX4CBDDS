...
/**
 * RU Report generation action.
 * 
 * @author smcfee
 *
 */
public class RUReportAction implements IObjectActionDelegate {
	...
	public void run(IAction action) {		
		...
		try {
			...
			IRunnableWithProgress op = new IRunnableWithProgress() {
				...
				public void run(IProgressMonitor monitor)
					...
					monitor.setTaskName("Generating resource utilization report.");
					...
					if( ZDLUtil.isZDLConcept(dsModel, DSDomainNames.DSMODEL)) {
						...
						location += "/" + ((NamedElement)BaseUtil.getEObjectFromSelection(selection)).getName() + ".ru.xml";
					}
		...
		Path path = new Path(srcGen + "/ru/resourceLog.html");
