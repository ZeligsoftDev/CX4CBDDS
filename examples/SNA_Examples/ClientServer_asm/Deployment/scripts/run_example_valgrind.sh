...
function quit 
{
   ...
   $SNA_ROOT/tools/scripts/sna_shutdown_components.py \
                                             ../descriptors/ClientServer_asm.cdp
...
##
## To run in valgrind, we are going to add some arguments to the script
##   type `valgrind --help' in a terminal to see the various arguments
##

START_NODES="$SNA_ROOT/tools/scripts/sna_start_nodes.py \
   ../descriptors/ClientServer_asm_VM.cdd \
   --mem-check-tool=valgrind \
   --mem-check-args=\"--leak-check=full \
   --xml=yes \
   --xml-file=%p.valgrind.xml\""
