...
public class ForEachStatement extends StatementWithBody {
	...
	public void analyzeInternal(XpandExecutionContext ctx, final Set<AnalysationIssue> issues) {
		...
		ctx = (XpandExecutionContext) ctx.cloneWithVariable(new Variable(getVariable().toString(), t));
		if (iteratorName != null) {
			ctx = (XpandExecutionContext) ctx.cloneWithVariable(new Variable(iteratorName.toString(), ctx.getTypeForName(IteratorType.TYPE_NAME)));
		}
	...
	public void evaluateInternal(XpandExecutionContext ctx) {
		...
		if (iteratorName != null) {
			ctx = (XpandExecutionContext) ctx.cloneWithVariable(new Variable(iteratorName.toString(), iterator));
		}
		for (final Iterator<?> iter = col.iterator(); iter.hasNext();) {
			final Object element = iter.next();
			ctx = (XpandExecutionContext) ctx.cloneWithVariable(new Variable(getVariable().toString(), element));
			ctx.preTask(this);
